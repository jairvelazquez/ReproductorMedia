 ...
 * Profesor: Jose Manuel Corona 
 * Elias Jair Velazquez Espinoza 
 */
// Enlace de prueba para video externo https://www.html5rocks.com/es/tutorials/video/basics/Chrome_ImF.mp4
public class ReproductorMedia extends JFrame implements ActionListener {
	...
	crono = new CronometroA();//Se inicializa el cronometro
	...
	filtroExtensiones = new FileNameExtensionFilter("Tipos de Media","mp3","mp4","mkv","avi","mpeg");//Formatos base aceptados por la libreria de VLC 
		
	reproductor = new EmbeddedMediaPlayerComponent();//Inicializamos el reproductor
	path = "";
    ...
    public ReproductorMedia () {
        ...
        sPATH_VLC = "C:/Program Files/VideoLAN/VLC";//Dirección donde suele instalarse el VCL Player, cambiar en caso de haber instalado el programa en otra ruta
        iniciaComponentes();
        iniciaHerramientas();
        ...
        * */
        ...
        BarraVolumen.addChangeListener(new ChangeListener() {//Declaramos un ChangeListener para escuchar el movimiento de la barra de volumen
        
        @Override
        public void stateChanged(ChangeEvent e) {//Eventos de cambio 
            Object source = e.getSource();  
                if(volumenTiempo.isSelected()==true) {//Si el cambio pertenece a la barra de volumen
                    if(antesBarraTiempo!=0) {//Y fue un cambio diferente al anterior
                	BarraVolumen.setValue(estadoVolumen);//Se cambia la barra de volumen al estado seleccionado
                	antesBarraTiempo=0;//se declarada estado cero para la siguiente modificacion
                    }
                    reproductor.getMediaPlayer().setVolume( ((JSlider) source).getValue() );//Se obtiene el valor del slider
                    currentVolume = ((JSlider) source).getValue();//El controlador del volumen en nuestra variable global tambien recibe el valor
                }
            }
        });
        ...
        public void positionChanged(MediaPlayer mp, float pos) {//Se controla el deslizador del tiempo en reproduccion
            	
            if(volumenTiempo.isSelected()==false) {//Si la casilla de volumen no esta seleccionada
              	antesBarraTiempo = 1;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (decision == 0) { //Si se tomo la decision de un archivo local
                    ...
                    //path = "https://www.quirksmode.org/html5/videos/big_buck_bunny.mp4";//Nueva Prueba	
